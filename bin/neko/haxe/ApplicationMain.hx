...
@:dox(hide) class ApplicationMain {
	...
	public static function main () {
		...
		var projectName = "coding-challenge";
		
		var config = {
			
			build: "58",
			company: "Andrew van der Westhuizen",
			file: "coding-challenge",
			fps: 60,
			name: "coding-challenge",
			orientation: "",
			packageName: "net.avdw.coding-challenge",
			version: "1.0.0",
			windows: [
		...
		#if (js && html5)
		#if (munit || utest)
		lime.system.System.embed (projectName, null, 1040, 1040, config);
		#end
		#else
		create (config);
		#end
	...
	macro public static function getEntryPoint () {
		...
		switch (Context.follow (Context.getType ("net.avdw.Main"))) {
			...
			case TInst (t, params):
				...
				if (hasMain) {
					...
					return Context.parse ("@:privateAccess net.avdw.Main.main ()", Context.currentPos ());
					...
					Context.fatalError ("Main class \"net.avdw.Main\" has neither a static main nor a constructor.", Context.currentPos ());
					
				}
				
			default:
				
				Context.fatalError ("Main class \"net.avdw.Main\" isn't a class.", Context.currentPos ());
...
@:keep @:dox(hide) class DocumentClass extends net.avdw.Main {}
